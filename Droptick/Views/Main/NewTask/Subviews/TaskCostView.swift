...
struct TaskCostView: View {
    ...
    func minutesSinceLastMarker() -> Int? {
        ...
        let since = Int(Date.now.timeIntervalSince(lastMarkerDate!) / 60)
        if since < 1 {
            return nil
        }
        return since
    ...
    func minutesSinceLastTask() -> Int? {
        let todos = viewModel.todos()
        if todos.count == 0 {
            return nil
        }
        return Int(Date.now.timeIntervalSince(todos[0].createdAt) / 60)
    }
}
