...
//    var onToggleSetReminder: (() -> Void)?
    ...
    var body: some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(
                    LinearGradient(gradient: Gradient(colors: [.orange, .red, .red]), startPoint: .topLeading, endPoint: .bottomTrailing)
                )
//                .foregroundColor(tapped ? .white : .red.opacity(tapped ? 0.9 : 1))
                .frame(height: 60)
